<div id="p<?php echo context::global_filters(dcCore::app()->ctx->posts->post_id,array (
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'EntryID'); ?>" class="post short <?php echo ((dcCore::app()->ctx->posts->index()+1)%2 ? "odd" : ""); ?> <?php if (dcCore::app()->ctx->posts->index() == 0) { echo 'first'; } ?>" lang="<?php if (dcCore::app()->ctx->posts->post_lang) { echo context::global_filters(dcCore::app()->ctx->posts->post_lang,array (
  0 => NULL,
  'encode_xml' => 0,
  'encode_html' => 0,
  'cut_string' => 0,
...
dcCore::app()->ctx->meta = dcCore::app()->meta->getMetaRecordset(dcCore::app()->ctx->posts->post_meta,'tag'); dcCore::app()->ctx->meta->sort('meta_id_lower','asc'); ?><?php while (dcCore::app()->ctx->meta->fetch()) : ?>
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'EntryTitle'); ?>"><?php echo __('Continue reading'); ?></a>
		</p>
	<?php endif; ?>
  ...
  'capitalize' => 0,
  'strip_tags' => 0,
),'EntryIfContentIsCut')) > strlen(context::global_filters(dcCore::app()->ctx->posts->getContent(0),array (
  0 => NULL,
  'encode_xml' => 0,
  'encode_html' => '1',
  'cut_string' => '300',
